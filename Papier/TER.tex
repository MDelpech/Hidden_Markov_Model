...
\hspace*{0.5cm}2.1) Définitions et propriétés d'une chaîne de Markov \\
\hspace*{1cm}2.1.1) Définitions \\
\hspace*{1cm}2.1.2) Propriétés \\
\hspace*{0.5cm}2.2) Définition d'une chaîne de Markov cachée. \\
...
\hspace*{0.5cm}3.1) Estimation de la vraisemblance et algorithme Forward \\
\hspace*{0.5cm}3.2) Décodage et algorithme de Viterbi \\
...
\hspace*{1cm}4.1.4) Évaluation hors échantillon\\
...
\hspace*{0.5cm}5.4) Détection de régimes de volatilité sur le VIX \\
...
\subsection{Définitions et propriétés d'une chaîne de Markov}
...
Les chaînes de Markov cachées (HMM) sont des modèles statistiques utilisés dans une situation où on a des états latents qui ne sont pas directement observables. Les données observables sont générées par ces états suivant une loi de probabilité conditionnelle.
...
\subsubsection{Lois jointes et vraisemblance}

\textbf{Propriété :}
Soit $X$ = ($X_1$, \dots, $X_T$) la séquence d'états cachés et $Y$ = ($Y_1$, \dots, $Y_T$) la séquence d'observations, la loi des observations conditionnellement à la séquence d'états cachés est : 
\[
...
\textbf{Propriété :}
Soit $X$ = ($X_1$, \dots, $X_T$) la séquence d'états cachés et $Y$ = ($Y_1$, \dots, $Y_T$) la séquence d'observations, la loi jointe des observations et des états cachés est donnée par : 
\[
...
\textbf{Remarque :}
La loi des observations à chaque instant $t$ dépend de l'état caché à cet instant et pas des états d'avant. 
\vspace{0.3cm}

\textbf{Exemple :}
Soit un modèle de chaîne de Markov cachée avec deux états $A$ et $B$. 
En supposant que les observations sont gaussiennes conditionnellement à l'état caché, on a :
\[
...
\textbf{Remarque :}
$X_t$ agit comme un sélecteur de distribution pour $Y_t$.
\vspace{0.3cm}

\subsubsection{Exemple global d'une chaîne de Markov cachée}
...
\vspace{0.5cm}
\textbf{NB :}
La matrice de transition $A$ n'est pas connue dans la plupart des applications, elle doit être estimée à partir des données d'observations. Par exemple avec l'algorithme de Baum-Welch que nous verrons plus tard.
\vspace{1.5cm}
...
\textbf{Propriété :}
La probabilité totale des observations $(Y_t)_{t}$ est donnée par : 
\[
...
\textbf{Propriété :}
Dans un HMM non dégénéré, le processus d'observations $(Y_t)_{t}$ n'est pas un processus de Markov, même si les états cachés $(X_t)_{t}$ le sont. 
...
\subsection{Estimation de la vraisemblance et algorithme Forward}

\textbf{Problématique :}
Comment calculer efficacement la vraisemblance $\mathbb{P}(Y)$ d'une séquence d'observations $Y$ donnée par un modèle de HMM $(A, B, \pi)$ ? \\ 
...
Cet algorithme repose sur le concept de programmation dynamique et utilise une variable intermédiaire $\alpha_t(i)$ qui représente la probabilité d'observer la séquence d'observations $y_1 \dots y_t$ et d'être dans l'état caché $i$ à l'instant $t$.\\
...
\subsection{Décodage et algorithme de Viterbi}

Considérons un nouveau problème : le décodage d'une suite d'états d'après une série d'observations $Y_1,\dots,Y_T$.
On essaye de découvrir la partie cachée du modèle.\\
i.e. Découvrir a posteriori la ``vraie'' séquence d'états $X = (X_1, \dots, X_T)$ qui s'est produite pour générer une série d'observations, sachant la distribution initiale, la matrice de transition ainsi que les probabilités d'émission.
...
Une première approche qu'on pourrait qualifier de ``Greedy'' serait de choisir, à chaque instant $t$, l'état $X_t$ tel que la probabilité d'observer $Y_t$ soit la plus grande.
Ce critère d'optimalité maximise l'espérance du nombre d'états correctement décryptés un à un.

Cependant, cela peut amener plusieurs problèmes avec la séquence d'états qui en résulte. Comme le critère local ignore la matrice de transition, il peut proposer une transition de probabilité nulle.
...
Il va donc falloir trouver un critère prenant en compte la probabilité de réalisation d'une séquence d'états. On pourrait ainsi vouloir maximiser l'espérance du nombre de paires correctes d'états découverts, ou de triplets, etc.
Cependant, le critère le plus utilisé est le critère de trouver l'unique séquence d'états qui maximise la probabilité de voir nos observations sachant la distribution initiale, la matrice de transition ainsi que les probabilités d'émission.
...
\begin{itemize}
    ...
    \item chaque nœud $(t,j)$ représente le fait d'être dans l'état $j$ au temps $t$.
    \item chaque arête entre $(t-1,i)$ et $(t,j)$ représente une transition possible, pondérée par la proba $a_{ij}$.
    \item chaque nœud est également associé à la proba d'émission $b_j(y_t)$.
\end{itemize}
...
Chaque chemin correspond à une séquence d'états cachés possibles.
...
L'intérêt du treillis est qu'il met en évidence une propriété fondamentale : pour retrouver la séquence d'états, il faut garder en mémoire l'argument qui maximise la récurrence. Nous allons le faire via la liste $\psi_t(j)$.
...
\Statex
\Statex \textbf{Retraçage du chemin}
...
On obtient ainsi avec $X_1^\star,\dots,X_T^\star$ notre séquence d'états recherchée.

\vspace{0.3cm}

\textbf{Analyse de la complexité :}
\begin{itemize}
    \item À chaque instant, on calcule une valeur pour chacun des $N$ états.
...
Il y a $T$ instants, on obtient donc une complexité temporelle totale de :
\[
O(TN^2)
\]
...
Avant toute chose, il est important de noter que cette méthode itérative permet d'approximer un optimum local à notre problème.
...
\textbf{$\bullet$} $\xi_t(i,j)$ := la probabilité d'être dans l'état $i$ à l'instant $t$ et dans l'état $j$ à l'instant $t+1$ sachant le modèle $(A,B,\pi)$ et la série d'observations $Y$.
...
\newpage
\textbf{$\bullet$} $\gamma_t(i)$ := la probabilité d'être dans l'état $i$ à l'instant $t$, sachant la série d'observations et le modèle $(A,B,\pi)$.
...
\]
\[
\pi_i' := \text{probabilité d'être dans l'état } i \text{ au moment } 1
=\gamma_1(i)
\]
...
{\text{espérance du nombre de fois où on est dans l'état } j}
\]
\[
=
...
Dans le cas où les observations sont continues, alors on estime les paramètres avec des estimateurs naturels issus de ces quantités.
...
Pour pallier ce problème, on peut alors effectuer une modification de l'algorithme de forward en passant à la log-vraisemblance. Cela permet de transformer les produits en sommes, le tout avec des valeurs plus grandes et par conséquent plus faciles à manipuler numériquement.\\
...
\right|.
\]
Deux estimations peuvent avoir des vraisemblances proches mais des régimes de persistance très différents.
...
La matrice d'émission est quant à elle définie par :
\[
B =
\begin{pmatrix}
0.1 & 0.9 \\
...
Cela nous permettra notamment de tester la fiabilité des différents algorithmes.

\subsection{Application des algorithmes sur données simulées}\mbox{}\\
...
\begin{figure}[h]
    ...
    \caption{Détection de régime sur l'indice du VIX entre 2004 et 2024}
    \label{fig:mon_image}
\end{figure}
...
\bibitem{bickel1998}
Bickel, P. J., Ritov, Y., \& Rydén, T. (1998).
\emph{Asymptotic normality of the maximum-likelihood estimator for general hidden Markov models}.
The Annals of Statistics, 26(4), 1614--1635.
\url{https://doi.org/10.1214/aos/1024691255}

\bibitem{viterbi1967}
Viterbi, A. J. (1967).
\emph{Error bounds for convolutional codes and an asymptotically optimum decoding algorithm}.
IEEE Transactions on Information Theory, 13(2), 260--269.
\url{https://doi.org/10.1109/TIT.1967.1054010}

\bibitem{bahl1974}
Bahl, L. R., Cocke, J., Jelinek, F., \& Raviv, J. (1974).
\emph{Optimal decoding of linear codes for minimizing symbol error rate}.
IEEE Transactions on Information Theory, 20(2), 284--287.
\url{https://doi.org/10.1109/TIT.1974.1055186}
...
\bibitem{celeux2008}
Celeux, G., \& Durand, J.-B. (2008).
\emph{Selecting hidden Markov model state number with cross-validated likelihood}.
Computational Statistics, 23(4), 541--564.
\url{https://doi.org/10.1007/s00180-007-0097-1}

\bibitem{hamilton1989}
Hamilton, J. D. (1989).
\emph{A new approach to the economic analysis of nonstationary time series and the business cycle}.
Econometrica, 57(2), 357--384.
\url{https://doi.org/10.2307/1912559}

\bibitem{ryden1998}
Rydén, T., Teräsvirta, T., \& Åsbrink, S. (1998).
\emph{Stylized facts of daily return series and the hidden Markov model}.
Journal of Applied Econometrics, 13(3), 217--244.
\url{https://doi.org/10.1002/(SICI)1099-1255(199805/06)13:3<217::AID-JAE476>3.0.CO;2-V}
