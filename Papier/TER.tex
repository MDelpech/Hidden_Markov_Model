...
\]
Plus \(D_{VP}\) est proche de \(0\), plus les deux méthodes donnent des régimes similaires. Une valeur élevée indique au contraire que le choix des états est incertain ou dépend fortement de la méthode de décodage. À l'inverse, si les deux méthodes donnent des résultats très différents, cela révèle une incertitude importante sur les états cachés. Le chemin Viterbi doit alors être interprété comme une trajectoire probable, et non comme une classification certaine.
...
\paragraph{Comparaison avec le posterior decoding et le décodage local}\mbox{}\\
...
\]
Le décodage local par émission choisit, à chaque date, l'état qui rend l'observation courante la plus probable :
\[
\widehat X_t^E=\arg\max_i b_i(Y_t).
\]
On compare les trois décodages avec le taux de désaccord
\[
D_{VP}
=\frac{1}{T}\sum_{t=1}^T
\mathbf 1_{\{\hat X^V_t\neq \hat X^P_t\}}.
