...
Afin de tester et de valider les différents algorithmes étudiés dans ce travail, nous avons construit un jeu de données simulé à partir d'un modèle de Markov caché. L'objectif de cette simulation est de disposer d'une séquence d'observations pour laquelle les paramètres du modèle sont connus à l'avance. Cela permet ensuite de vérifier si les algorithmes d'inférence et d'estimation parviennent à retrouver correctement la structure sous-jacente du modèle.
...
La dynamique de la chaîne de Markov cachée est décrite par la matrice de transition
\[
A =
\begin{pmatrix}
0.7 & 0.295 & 0.005 \\
...
\end{pmatrix}.
\]
Chaque coefficient $A_{ij}$ représente la probabilité de passer de l'état caché $i$ à l'état caché $j$ entre deux instants successifs. Par exemple, si le système est dans l'état \og soleil \fg, il reste dans cet état avec probabilité $0.7$, passe à l'état \og nuageux \fg{} avec probabilité $0.295$, et passe à l'état \og pluie \fg{} avec probabilité $0.005$.

La matrice d'émission, qui donne les probabilités d'observer chaque symbole conditionnellement à l'état caché courant, est définie par
\[
B =
\begin{pmatrix}
0.1 & 0.9 \\
...
\end{pmatrix}.
\]
Ainsi, lorsque l'état caché est \og soleil \fg, la probabilité d'observer \og parapluie \fg{} est faible, tandis que lorsque l'état caché est \og pluie \fg, cette probabilité devient élevée. Cette matrice permet donc de créer un lien probabiliste cohérent entre les états météorologiques cachés et les observations visibles.
...
Ce jeu de données simulé constitue un cadre contrôlé pour évaluer les algorithmes. En effet, contrairement à un jeu de données réel, les états cachés et les vrais paramètres du modèle sont connus. Il est donc possible de comparer les résultats produits par les algorithmes avec les valeurs utilisées pour générer les données. Cela permet notamment de tester la capacité des méthodes comme l'algorithme Forward, l'algorithme de Viterbi ou encore l'algorithme de Baum-Welch à retrouver la vraisemblance, la trajectoire cachée la plus probable ou les paramètres du modèle.
...
\begin{figure}[h]
    \centering
    \includegraphics[width = 1\textwidth]{/Users/max.dlch/Desktop/M1/S2/Hidden_Markov_Model/Papier/Figures/regime_detectionVIX.png}
