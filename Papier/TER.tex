...
\subsection{Simulation d'un HMM et application à des données financières}
...
\subsubsection{Vraisemblance et log-vraisemblance}
...
La vraisemblance brute est très faible, ce qui est attendu pour une longue suite d'observations. On
utilise donc la log-vraisemblance
\[
...
Le modèle de simulation minimise l'AIC et le BIC. Les modèles à émissions uniformes obtiennent
une log-vraisemblance moyenne proche de \(-\log(2)\), ce qui indique que supprimer le lien entre
états cachés et observations dégrade fortement la qualité forward.
...
La série est découpée chronologiquement en 800 observations d'apprentissage et 201 observations de
test. La performance hors échantillon est mesurée par
\[
\ell_{\mathrm{test}}
...
-0.639175, la NLPD vaut 0.639175, le modèle générateur minimise l'AIC et le BIC parmi les modèles
comparés, et la NLPD hors échantillon reste proche de celle obtenue sur la période d'apprentissage.
La comparaison avec les modèles uniformes montre en particulier que la matrice d'émission joue un
rôle central : lorsque le lien entre états cachés et observations est supprimé, la vraisemblance se
dégrade nettement.
...
\subsubsection{Interprétation des régimes détectés}
...
La Figure~\ref{fig:stabilite_pi} montre la stabilité de la distribution initiale $\pi$. Les écarts obtenus sont quasiment nuls, avec des valeurs de l'ordre de $10^{-195}$. Numériquement, cela signifie que les différentes initialisations conduisent pratiquement à la même estimation de la distribution initiale. Ce résultat doit toutefois être interprété avec prudence : sur une trajectoire longue, la distribution initiale influence surtout le début de la séquence et a donc un impact limité sur la vraisemblance totale. Il est donc normal qu'elle soit moins informative que les matrices $A$ et $B$ pour juger de la qualité globale de l'estimation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Test/stabilite_pi.png}
    \caption{Stabilité de la distribution initiale estimée.}
    \label{fig:stabilite_pi}
\end{figure}

Les Figures~\ref{fig:stabilite_A} et~\ref{fig:stabilite_B} montrent respectivement la stabilité de la matrice de transition $A$ et de la matrice d'émission $B$. Contrairement à la distribution initiale, ces deux matrices présentent des écarts plus marqués selon l'initialisation. On observe notamment que les initialisations 1, 2, 6, 7 et 8 conduisent à des distances relativement importantes par rapport à la meilleure solution. Cela montre que les paramètres estimés ne sont pas parfaitement stables.
...
\subsubsection{Détection de régimes de volatilité sur le VIX}
...
\[
    ...
    \end{pmatrix}.
\]
Ces variables sont ensuite standardisées avant l'estimation du modèle, afin que les différentes composantes aient des ordres de grandeur comparables. Cette étape est importante car l'algorithme d'estimation repose sur des densités gaussiennes multivariées et peut être sensible aux différences d'échelle entre les variables.
...
\]
où $\mu_i$ est le vecteur moyen associé au régime $i$ et $\Sigma_i$ sa matrice de covariance. La dynamique temporelle des états cachés est décrite par une chaîne de Markov homogène de matrice de transition
\[
    A = (a_{ij})_{0 \leq i,j \leq 2},
\]
où
\[
...
L'estimation du modèle est réalisée à l'aide de l'algorithme de Baum-Welch, qui correspond à une procédure EM adaptée aux modèles de Markov cachés. À chaque itération, l'algorithme estime les probabilités a posteriori des états cachés, puis met à jour les paramètres du modèle afin d'augmenter la vraisemblance des observations. Une fois le modèle estimé, la séquence la plus probable des états cachés est obtenue par l'algorithme de Viterbi.
...
\end{array}
\]
Ces résultats sont cohérents avec l'interprétation attendue. Le régime calme correspond à un niveau moyen du VIX relativement faible, autour de $16.38$, et à une faible volatilité réalisée. Le régime intermédiaire présente un niveau moyen du VIX plus élevé, autour de $20.02$, avec une volatilité réalisée également plus importante. Enfin, le régime de crise est associé au niveau moyen du VIX le plus élevé, autour de $23.67$, ainsi qu'à la volatilité réalisée moyenne la plus forte.
...
\[
    ...
    \end{pmatrix}.
\]
On observe que les coefficients diagonaux sont élevés. Cela signifie que les régimes sont persistants : lorsqu'un régime est atteint, il a une forte probabilité de se maintenir au jour suivant. Par exemple, la probabilité de rester dans le régime calme est estimée à environ $97.2\%$, tandis que la probabilité de rester dans le régime de crise est d'environ $94.7\%$. Cette persistance est cohérente avec l'idée que les phases de marché ne changent généralement pas brutalement d'un jour à l'autre, mais s'inscrivent dans des périodes de plusieurs jours ou semaines.
...
On remarque que le modèle identifie de nombreuses phases calmes, représentées par le régime de plus faible volatilité, mais qu'il bascule vers les régimes intermédiaire et de crise lors des épisodes de forte instabilité. Ces changements de régime apparaissent notamment autour des grandes hausses du VIX, par exemple pendant la crise financière de 2008, certaines tensions de marché autour de 2011--2012, la période 2015--2016, ainsi que le choc de volatilité observé en 2020. Le modèle semble donc capable de détecter automatiquement des périodes de stress de marché à partir des variables construites sur le VIX.

Cependant, il faut souligner une limite importante de cette approche. Le modèle HMM ne donne pas directement une explication économique des régimes : il segmente la série temporelle en fonction des propriétés statistiques des observations. Ainsi, le régime de crise n'est pas défini par un événement économique particulier, mais par une combinaison de rendements et de volatilités réalisées plus élevées. L'interprétation économique des états cachés doit donc être faite a posteriori, en comparant les régimes estimés avec les périodes connues de tension sur les marchés.

De plus, les régimes estimés dépendent du choix des variables utilisées, du nombre d'états cachés et de l'initialisation de l'algorithme. Le choix de trois régimes permet ici une interprétation naturelle en termes de marché calme, intermédiaire et crise, mais d'autres choix pourraient être envisagés. Une étude plus complète pourrait comparer plusieurs modèles avec différents nombres d'états, par exemple à l'aide de critères d'information comme l'AIC ou le BIC, ou encore en évaluant la capacité du modèle à détecter des périodes de stress sur un échantillon hors apprentissage.

Cette application illustre néanmoins l'intérêt des modèles de Markov cachés pour l'analyse de séries financières. Ils permettent de modéliser explicitement une dynamique non observable, ici le régime de marché, tout en conservant une structure probabiliste simple et interprétable. Dans le cas du VIX, le HMM fournit une méthode automatique de classification des périodes de marché selon leur niveau de volatilité et leur persistance.
